...
- **Degree labels are the source of truth for both display modes**: each preset carries explicit degree labels; note-name labels are derived from the root spelling and degree label via `spellDegree`. Custom scales fall back to a 12-entry flat-leaning map, upgraded by a greedy letter-preference pass that ties toward flats.
...
- **Shared marker-building function**: `buildMarkers(config, root, intervals, displayMode)` is the single function that turns a root + interval set into `Marker[]`. Both the scale and (later) arpeggio tools call this function rather than each building markers independently.
...
- [Custom scale degree labeling is ambiguous without a key context] → flat-leaning fallback map with a greedy per-letter upgrade pass keeps labels readable without requiring the user to pick a key
